...
drop policy if exists "member event registration read own" on public.event_registrations;
...
create policy "event registrations manager insert"
...
create policy "event registrations manager update"
...
create policy "event registrations manager delete"
...
create policy "schedules manager insert"
on public.volunteer_schedules for insert to authenticated
with check (
  private.has_permission(church_id,'ministries.manage')
  and exists (
    select 1 from public.church_members m
...
create policy "schedules manager update"
...
create policy "schedules manager delete"
on public.volunteer_schedules for delete to authenticated
using (private.has_permission(church_id,'ministries.manage'));
