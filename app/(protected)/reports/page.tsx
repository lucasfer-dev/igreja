import { CalendarDays, MessageCircle, Sparkles, TrendingUp, Users, UsersRound } from 'lucide-react';
...
export default async function ReportsPage(){
  ...
  const [members,visitors,events,cells,attendance,newMembers,newVisitors,followups]=await Promise.all([
    ...
    supabase.from('whatsapp_outbox').select('status').eq('church_id',churchId).gte('created_at',since.toISOString())
  ]);
  ...
  const months=Array.from({length:6},(_,i)=>{
    ...
    return {label:d.toLocaleDateString('pt-BR',{month:'short'}).replace('.',''),memberGrowth,visitorGrowth,presence};
  });
  const maxPresence=Math.max(1,...months.map(m=>m.presence));
  const sent=(followups.data||[]).filter(x=>x.status==='sent').length;

  return <>
    <header className="module-heading"><div><span className="module-kicker">Inteligência</span><h1>Relatórios</h1><p>Crescimento, participação e relacionamento em uma visão operacional.</p></div></header>
    <section className="report-domain-grid">
      ...
      <article><MessageCircle size={20}/><div><strong>{sent}</strong><span>Follow-ups enviados</span><small>últimos 6 meses</small></div></article>
    </section>
