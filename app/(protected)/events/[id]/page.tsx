...
export default async function EventDetail({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{error?:string;message?:string}>}){
  ...
  const {supabase,churchId,roleKey,user,churchName}=context;
  ...
  const [{data:registrations},{data:schedules}]=await Promise.all([
    canManage
      ...
      : member?.id
        ? supabase.from('event_registrations').select('id,status,checked_in_at,member_id').eq('church_id',churchId).eq('event_id',id).eq('member_id',member.id).limit(1)
  ...
  const isRegistered=Boolean(member?.id&&registrations?.some(r=>r.member_id===member.id));
  const register=registerForEvent.bind(null,id);
  const checked=canManage?(registrations?.filter(r=>r.checked_in_at).length||0):0;
  ...
  if(isMember){
    return <div className="member-event-detail">
      <Link className="member-back-link" href="/events">← Voltar para eventos</Link>
      <section className="member-event-detail-hero">
        <div className="member-event-detail-media" style={event.banner_url?{backgroundImage:`linear-gradient(180deg,rgba(82,36,2,.08),rgba(82,36,2,.78)),url("${event.banner_url}")`}:undefined}>
          <span>{event.category||'Evento'}</span>
        </div>
        <div className="member-event-detail-copy">
          <span className="page-kicker">Agenda da igreja</span>
          <h1>{event.title}</h1>
          <div className="member-event-detail-meta">
            <span><CalendarDays size={16}/>{new Date(event.starts_at).toLocaleString('pt-BR',{dateStyle:'long',timeStyle:'short'})}</span>
            {event.address&&<span><MapPin size={16}/>{event.address}</span>}
          </div>
          <p>{event.description||'Confira as informações deste evento e participe com a nossa comunidade.'}</p>
          {qs.error&&<p className="alert">{qs.error}</p>}
          {qs.message&&<p className="success-alert">{qs.message}</p>}
          <div className="member-event-detail-actions">
            <ShareEventActions title={event.title} text={invite} whatsappUrl={whatsappLink(null,invite)}/>
            {!isRegistered&&<form action={register}><button className="module-primary" type="submit">Quero participar</button></form>}
            {isRegistered&&<span className="success-chip"><CheckCircle2 size={16}/> Inscrição confirmada</span>}
          </div>
        </div>
      </section>
    </div>;
  ...
  return <>
    ...
    <header className="event-workspace-hero" style={event.banner_url?{backgroundImage:`linear-gradient(90deg,rgba(255,255,255,.96),rgba(255,255,255,.86)),url("${event.banner_url}")`,backgroundSize:'cover',backgroundPosition:'center'}:undefined}>
      ...
      <div className="quick-actions"><ShareEventActions title={event.title} text={invite} whatsappUrl={whatsappLink(null,invite)}/></div>
    </header>
    ...
    {canManage&&<>
      ...
      <section className="workspace-grid">
        <div className="stack"><section id="registrations" className="panel"><div className="section-title"><div><span className="section-eyebrow">Participantes</span><h2>Inscrições</h2></div><span className="section-count">{registrations?.length||0}</span></div>{registrations?.length?<div className="event-registration-list">{registrations.map((r:any)=>{const checkin=checkInRegistration.bind(null,id,r.id);return <div key={r.id}><span className="person-dot">{(r.full_name||'P').slice(0,1)}</span><div><strong>{r.full_name||'Participante'}</strong><span>{r.phone||r.email||'Sem contato'}</span></div><span className="soft-status">{r.status}</span>{r.checked_in_at?<span className="checked-time">✓ {new Date(r.checked_in_at).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}</span>:<form action={checkin}><button className="small-action" type="submit">Check-in</button></form>}</div>})}</div>:<div className="empty compact">Nenhuma inscrição ainda.</div>}</section></div>
