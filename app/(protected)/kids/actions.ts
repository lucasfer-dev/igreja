...
export async function addGuardian(childId:string,formData:FormData){
 const fullName=String(formData.get('full_name')||'').trim();if(!fullName)return;const {supabase,churchId}=await requirePermission('kids.manage');
...
export async function checkInChild(childId:string,formData:FormData){
 const {supabase,churchId,user}=await requirePermission('kids.manage');const securityCode=String(randomInt(100000,1000000));
...
export async function checkOutChild(checkinId:string,formData:FormData){
 const code=String(formData.get('security_code')||'').trim();const guardianId=String(formData.get('guardian_id')||'');const {supabase,churchId}=await requirePermission('kids.manage');
