...
export default async function ServiceOrderPage({searchParams}:{searchParams:Promise<{error?:string}>}){
  ...
  return <>
    <header className="module-heading">
      <div><span className="module-kicker">Cultos & operação</span><h1>Ordem do culto</h1><p>Organize tudo o que vai acontecer no culto, em sequência, com duração e responsáveis.</p></div>
    ...
    <section className="workspace-grid">
      ...
      <aside id="new-order" className="panel">
        <div className="section-title"><div><span className="section-eyebrow">Nova ordem</span><h2>Preparar um culto</h2></div><UsersRound size={18}/></div>
        <form action={createServiceOrder} className="form">
          <div className="field"><label>Nome</label><input name="title" placeholder="Ex.: Culto de Domingo — Noite" required/></div>
          ...
          <button className="primary-submit" type="submit"><Plus size={15}/> Criar ordem do culto</button>
        </form>
      </aside>
    </section>
  </>;
